...
export default function Booking() {
  ...
  // Pre-populate details from active auth session to secure parameters & prevent casing/typo mismatches
  useEffect(() => {
    if (user) {
      setFormData(prev => ({
        ...prev,
        email: user.email?.toLowerCase().trim() || prev.email,
                    ...
                    <CardContent className="p-8">
                      ...
                      <form id="booking-form" onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-2">
                          ...
                          <Input required value={formData.full_name} onChange={e => setFormData({ ...formData, full_name: e.target.value })} placeholder="Jane Doe" className="text-black" />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            <Input 
                              ...
                              readOnly={!!user}
                              className={cn(user ? "bg-secondary-50 cursor-not-allowed text-secondary-400" : "")}
                            />
                            {user && <p className="text-[10px] text-secondary-400">Authenticated via: {user.email}</p>}
                          </div>
                          <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea
                            className="flex w-full rounded-md border border-secondary-200 bg-white px-3 py-2 text-sm text-black ring-offset-background placeholder:text-secondary-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300 min-h-[100px]"
