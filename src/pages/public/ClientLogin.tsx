...
export default function ClientLogin() {
    ...
    try {
      ...
      if (isSignUp) {
        const response = await fetch('/api/auth/register', {
          ...
          body: JSON.stringify({ email: cleanEmail, password, role: 'user' })
        });
        ...
        const data = await response.json();
        if (!data.session) {
          throw new Error('Authentication response did not return a session');
        }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0A0A0A] text-[#FDFCFB] font-sans items-center justify-center p-6">
      <div className="w-full max-w-md bg-neutral-900 border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl">
        <div className="text-center mb-8">
          <h1 className="font-serif text-3xl italic mb-3">
            {isSignUp ? 'Create an Account' : 'Client Portal'}
          </h1>
          <p className="text-[10px] text-white/50 tracking-widest uppercase">
            {isSignUp ? 'Join to manage your events' : 'Sign in to view your bookings'}
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] tracking-widest uppercase text-white/70">Email</label>
            <input
              type="email"
              required
              value={email}
          ...
          <div className="space-y-2">
            <label className="text-[10px] tracking-widest uppercase text-white/70">Password</label>
            <input
              type="password"
              required
              value={password}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-4 mt-2 bg-[#D4AF37] text-black rounded-full text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-[#C5A059] transition-all disabled:opacity-50 flex justify-center items-center"
          >
        ...
        <div className="mt-8 text-center">
          <button
            ...
            className="text-[10px] text-white/50 hover:text-white uppercase tracking-widest transition-colors"
          >
