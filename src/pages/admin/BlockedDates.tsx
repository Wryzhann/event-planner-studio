...
export default function BlockedDates() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAuditEvent('BLOCKED_DATE_ADDED', `Blocked studio date: ${newDate} for: "${newReason || 'No reason specified'}"`);
  ...
  const handleRemove = async (id: string) => {
    try {
      const existing = blockedDates.find(d => d.id === id);
      const { error } = await supabase.from('blocked_dates').delete().eq('id', id);
      if (error) throw error;
      await logAuditEvent('BLOCKED_DATE_REMOVED', `Unblocked studio date: ${existing?.blocked_date || id}`);
